...
**Load evidence capture:** 2026-09-07T14:31:08Z

**Decision:** retain Query `16 / 16 / 30,000` and accept Corpus Mutation `2 / 2 / 1,000`
...
addressable client submissions in one trusted organization, while expensive work
is bounded to 16 Query and two Corpus Mutation claims. It also does not transfer
...
|---|---|---|---|---|---|---|---|---|
| P0-01 | Concurrent acceptance, replay, changed-input conflict; Answer/Retrieval Query and Corpus Mutation | Same submission key raced, then fingerprint changed | One queued row; replay returns it; changed input conflicts; full fuse inserts no row | Atomic idempotency and no duplicate effect | Retry the same key/input or choose a new key for new input | `test_answer_runs_pg.py::TestCreation`, `test_retrieval_runs_pg.py::test_retrieval_acceptance_claim_and_terminal_result_use_the_generic_store`, `test_run_runtime_lane_independence_pg.py::test_atomic_replay_conflict_authorization_and_lane_local_fuses`, `make load-runtime` | PASS | Caller owns stable keys; Operational State availability is infrastructure-owned |
...
| P0-04 | Duplicate writer replicas and Workspace ownership; Corpus Mutation lane | Six concurrent claimers compete at active ceiling two; two same-Workspace rows plus free Workspaces | Two claims on distinct Workspaces; same-Workspace successor remains queued | Deployment-wide active cap and one owning mutation per Workspace | Finish/release/defer the head; then FIFO successor becomes eligible | `test_run_runtime_lane_independence_pg.py::test_duplicate_workers_respect_active_caps_workspace_fifo_and_lane_independence` | PASS | Replica placement is infrastructure-owned |
...
|---|---|---|---|---|---|---|
| P1-01 | Query pressure must not starve mutation; mutation pressure must not starve Query | Fill one lane's reduced fuse and active ceiling; the other lane still accepts/claims/progresses | Independent fuses, semaphores, and deployment active caps; no global Query-vs-mutation lock | `test_run_runtime_lane_independence_pg.py`, `make load-runtime` | PASS | Provider/LightRAG internal concurrency remains external/upstream |
| P1-02 | Deferred/repair work must release capacity without breaking barriers | Typed outage defers alpha, beta claims; waiting alpha blocks alpha successor; next attempt recovers same Run | Permit release, Workspace barrier retention, skip-ineligible head | `test_run_runtime_lane_independence_pg.py::test_defer_releases_capacity_preserves_barrier_then_recovers_same_run`, `test_corpus_mutation_runs_pg.py` | PASS | Dependency restoration is operator-owned |
...
| P1-05 | 10k addressable client control-plane pressure | 10,000 Query submissions plus 1,000 mutation submissions covering all five production actions and one fuse rejection; fake executors hold 16+2 active | No lost Run, duplicate terminal, unbounded executor, deadlock, or starvation; eventual drain | `make load-runtime` and `tests/unit/test_run_runtime_load_contract.py` | PASS | This is not provider, parser, network, or fleet capacity evidence |
| P1-06 | Retrieval fan-out shape | Deterministic top-level Retrieval requests cover 1/10/50/100 Workspaces | One Query permit owns the request; no 100-document paid ingest is fabricated | `make load-runtime` (`retrieval_workspace_shapes`) | PASS | Corpus content/quality is release-operator evidence |
...
Environment: MacBook Pro `Mac16,5`, Apple arm64, 16 logical CPUs, 48 GiB
RAM, Darwin 25.6.0; Python 3.14.7; PostgreSQL 18.6 in the local isolated
container; `uv` 0.9.3. The frozen capture completed at
`2026-09-07T17:47:03.745326+00:00`; a rerun regenerates the gitignored evidence
and must not be represented by these numbers without updating this section. No
database URL, credential, absolute local path, raw log, or external provider
...
- one additional mutation at the full 1,000 nonterminal fuse was rejected before insert;
- fake Query/Corpus executors emitted durable phase/events and held the 16/two active ceilings;
...
Runs, zero duplicate terminal Runs, total duration 127.625 s, accepted throughput
86.190 Runs/s, and post-offer drain 37.272 s.

| Operation | Samples | p50 | p95 | p99 | max |
|---|---:|---:|---:|---:|---:|
| accept | 11,000 | 3.257 ms | 21.408 ms | 35.967 ms | 289.538 ms |
| status/owner lookup | 592 | 0.652 ms | 3.622 ms | 6.006 ms | 9.308 ms |
| cancel | 29 | 5.971 ms | 25.614 ms | 27.082 ms | 27.082 ms |
| event-cursor page/reconnect | 400 | 0.627 ms | 0.673 ms | 0.744 ms | 4.004 ms |

Queue residence was measured, not gated: p50 28.838465 s, p95 49.407353 s,
p99 53.556507 s. Maximum event-loop lag was 131.105 ms. Process high-water
RSS grew 4,489,216 bytes from a 111,362,048-byte idle high-water mark. The
largest observed database connection count was 21. Fake executor in-flight
maxima were exactly 16 Query and two Mutation.
...
1,000-row mutation fuse (0.110 ms captured execution) and a hash anti-join for
Workspace FIFO (0.540 ms), while the forced-index structural test proves
`idx_dlightrag_runs_claim` and `idx_dlightrag_runs_mutation_fifo` remain usable.
No additional index is justified at this bounded scale.

## Capacity decision and limitations

Corpus Mutation `2 / 2 / 1,000` is retained and accepted as the validated
default. The campaign reached both active permits, filled and rejected at the
1,000 fuse before insert, kept Query responsive, preserved per-Workspace FIFO,
and drained. Raising the values would increase uncontrolled LightRAG/parser
pressure without evidence; lowering them would discard demonstrated safe
capacity. Query `16 / 16 / 30,000` remains accepted and was not reopened.
