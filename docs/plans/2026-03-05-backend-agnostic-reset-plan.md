# Backend-Agnostic Reset Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rewrite `scripts/reset.py` to use LightRAG's native `drop()` method instead of hardcoded PostgreSQL/AGE cleanup, making it work with any configured storage backend.

**Architecture:** Initialize RAGService to get a fully configured LightRAG instance, iterate its 12 storage attributes calling `drop()` on each, then clean DlightRAG-specific data (hash_index, local files). The script remains a standalone CLI tool.

**Tech Stack:** Python, asyncio, LightRAG storage API, DlightRAG RAGService/config

---

### Task 1: Rewrite reset.py core — replace hardcoded backends with drop()

**Files:**
- Modify: `scripts/reset.py`

**Context:**
- Design doc: `docs/plans/2026-03-05-backend-agnostic-reset-design.md`
- Current `reset.py` has 3 hardcoded cleanup functions: `_reset_tables()` (PostgreSQL), `_reset_graphs()` (AGE), `_reset_local()` (files)
- LightRAG instance (after `RAGService.create()`) has 12 storage attributes, each with `async drop() -> dict[str, str]`
- The LightRAG instance lives at: `service.ingestion.rag.lightrag` (RAGService → IngestionPipeline → RAGAnything → LightRAG)
- `DlightragConfig` loads from `.env` automatically — no need for manual `_load_env()`
...
def _reset_local(working_dir: Path, *, dry_run: bool) -> tuple[int, int]:
    ...
    action = f"{working_dir}: {file_count} files ({_format_size(total_bytes)})" if dry_run else (
...
Expected: Should print the 12 storage attributes with their class names (e.g. `PGKVStorage`, `PGVectorStorage`, `PGGraphStorage`) and local files summary. No data deleted.

**Step 3: Verify actual reset works**

Run: `uv run scripts/reset.py -y`

Expected: Each storage prints `success — data dropped` (or similar from LightRAG's `drop()` return). Local files cleaned.

**Step 4: Commit**

```bash
git add scripts/reset.py
git commit -m "refactor: make reset.py backend-agnostic via LightRAG drop()"
```
...
**Step 1: Test --keep-files flag**

Run: `uv run scripts/reset.py --dry-run --keep-files`

Expected: Storages listed but "Local files" section not shown.

**Step 2: Test --verbose flag**

Run: `uv run scripts/reset.py --dry-run --verbose`

Expected: Debug logging from RAGService initialization visible.

**Step 3: Test cancellation**

Run: `uv run scripts/reset.py` then type `no` at the prompt.

Expected: Prints "Cancelled." and exits with code 1.
